...
function Assert-ScopePolicy([string]$Root) {
    $policy=Load-Json (Get-RepoPath $Root '.agent/verification/scope-policy.json'); Assert-ExactProperties $policy @('schema_version','protected_path_patterns','approval_source','protected_scope_requires_approval') 'scope policy'; if ($policy.schema_version -ne '2.0.0' -or $policy.approval_source -ne 'task_spec.scope_envelope.approved_protected_paths' -or $policy.protected_scope_requires_approval -ne $true) { Fail 'scope policy is invalid' }; [void](Assert-StringArray $policy.protected_path_patterns 'scope protected path patterns' $true); $required=@('.agent/context/**','.agent/verification/**','.agent/maps/**','.agent/contracts/**','AGENTS.md','docs/AGENT_ENGINEERING.md','scripts/prepare-agent-task.ps1','scripts/compile-agent-task-spec.ps1','scripts/run-agent-verification.ps1','scripts/verify-agent-context.ps1','scripts/verify-postgres.ps1'); foreach ($pattern in $required) { if ($pattern -notin @($policy.protected_path_patterns)) { Fail "scope policy is missing trust-boundary protection: $pattern" } }
}
