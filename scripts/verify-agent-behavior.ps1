...
$script:CasesPath = if ([string]::IsNullOrWhiteSpace($CasesPath)) { Join-Path $Root '.agent/evals/behavioral-cases.json' } else { [System.IO.Path]::GetFullPath($CasesPath) }
...
function Assert-CaseInventory($Document) {
    ...
    foreach ($case in $cases) {
        ...
        $seen += [string]$case.id
        $taskAllowed=@('objective','acceptance_criteria','scope_include','scope_exclude','expected_outcome','expected_change_patterns','forbidden_change_patterns','required_gates','seed_unrelated_file'); $unknown=@($case.task.PSObject.Properties.Name | Where-Object { $_ -notin $taskAllowed }); if ($unknown.Count -gt 0) { Fail "behavioral task contains unknown field(s): $($unknown -join ',')" }
        foreach ($required in @('objective','acceptance_criteria','scope_include','scope_exclude','expected_outcome','expected_change_patterns','forbidden_change_patterns','required_gates')) { if (-not $case.task.PSObject.Properties[$required]) { Fail "behavioral task is missing '$required': $($case.id)" } }
...
function Assert-BehaviorResults($Document, [switch]$RealEvidence) {
    ...
    foreach ($result in $results) {
        Assert-ExactProperties $result @('schema_version','id','expected_outcome','actual_outcome','task_success','scope_violations','protected_decision_violations','required_gate_pass','adapter_exit_code','changed_paths','gate_results','environment_checks','evidence_refs') "behavioral result $($result.id)"
        ...
        Assert-ExactProperties $result.environment_checks @('adapter_exit_zero','final_message_present','expected_path_observed','forbidden_path_absent') "environment checks $($result.id)"
        if ($RealEvidence) {
