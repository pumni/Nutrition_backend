...
6. Run required verification, scope, freshness, and policy checks.
...
Completion requires passing applicable canonical gates, approved-scope verification, current source freshness, policy checks, and an evidence-based report. Trusted evidence is written by the canonical runner outside the target worktree.
