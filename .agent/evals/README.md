...
Use scripts/verify-agent-behavior.ps1 -SelfTest -CasesPath <control-plane-case-bank> to validate the external task inventory and result schema. It deliberately does not manufacture behavioral results. Real trials publish adapter/model metadata, a baseline commit, the exact case-bank SHA-256, per-case worktree evidence, gate logs, changed paths, hidden assertion results, and environment outcomes outside the repository.
