...
The trusted prepare phase accepts human Task Intent and an authoritative baseline commit, writes a compiled Task Spec outside the target worktree, and binds the scope ceiling, approvals, and risk floor. The verification runner consumes that immutable spec, checks baseline-to-target ancestry, derives context and executable gate IDs from the observed diff, and writes bounded evidence outside the target worktree. Protected paths remain fail-closed. Reports reference gate IDs and evidence identity; they do not define commands.
